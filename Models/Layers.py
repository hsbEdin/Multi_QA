...
#         return x
#     bern = torch.bernoulli((1 - p) * (x.data.new(x.size(0), x.size(2)).zero_() + 1)).long()
#
#     dropout_mask = Variable((1.0 / (1-p) * bern), requires_grad=False).long()
...
def seq_dropout(X, p=0, training=False):
    ...
    tmp = torch.rand(X.shape)
    tmp = Variable(tmp.cuda())
    mask = (tmp < keep_prob).float()
    #print("mask:\n",mask)
