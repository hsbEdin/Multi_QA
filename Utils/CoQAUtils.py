...
class BatchGen:
    ...
    def bertify(self, words):
        ...
        for word in words:
            if word == "^":
                x_bert_offsets.append([len(bpe), len(bpe) + len(word)])
                bpe.append('[SEP]')
                count += 1
                sign = False
            else:
        ...
        bpe.append('[SEP]')
        while len(bpe)>self.opt['max_featrue_length']:
            bpe.pop(-2)
        x_bert = self.bert_tokenizer.convert_tokens_to_ids(bpe)
        return x_bert, x_bert_offsets, bpe, count
    ...
    def __iter__(self):
        ...
        for datum in data:
            ...
            for i in range(qa_len):
                ...
                if self.opt['dataset'] == self.opt['dataset'] == 'coqa' or self.opt['dataset'] == 'quac':
                    cur_query_token = datum['qas'][i]['annotated_question']['word']
                else:
                    cur_query_token = datum['qas'][i]['question']
                ...
                if self.opt['dataset'] == 'coqa' or self.opt['dataset'] == 'quac':
                    all_doc_tokens = datum['annotated_context']['word']
                else:
                    all_doc_tokens = datum['context']
                ...
                for (doc_index, doc_span) in enumerate(doc_spans):
                    ...
                    cur_q, cur_q_offsets, bpe, _ = self.bertify(cur_q)
                    ...
                    # print("seq: ", len(seq))
                    x_bert, x_bert_offsets, _, pre_length = self.bertify(seq)
                    ...
                    if self.opt['dataset'] == 'coqa' or self.opt['dataset'] == 'quac':
                        ...
                        for t, off in enumerate(context_word_offsets):
                            if tok_start_position==off[0]:
                                s = t
                            if tok_start_position==off[1]:
                                s = t+1
                            if tok_end_position == off[0]:
                                e = t-1
                                break
                            if tok_end_position in off or (tok_end_position>off[0] and tok_end_position<off[1]):
                                e = t
                                break
                            if tok_end_position > context_word_offsets[t-1][1] and tok_end_position < off[0]:
                                e = t-1
                                break
                            if t == len(context_word_offsets)-1 and off[1]==tok_end_position-1:
                                e = t
                        tok_start_position = s
                        tok_end_position = e+1
                        # if datum['qas'][i]['annotated_answer']['word'][0].lower() in ['yes', 'no']:
                        #     answer = datum['qas'][i]['span_text']
                        # else:
                        #     answer = datum['qas'][i]['annotated_answer']['word']
                        answer = datum['qas'][i]['span_text']

                    else:
                        answer = datum['qas'][i]['answer']

                    ### 新的context_word_offsets
                    q_word = ['CLS'] + datum['qas'][i]['annotated_question']['word'] + ['SEP']
                    q_text = 'CLS ' + datum['qas'][i]['question'] + ' SEP'
                    q_offsets = self.get_raw_context_offsets(q_word, q_text)
                    p = q_offsets[-1][1] + 1
                    new_offsets = []

                    for off in context_word_offsets:
                        new_offsets.append((off[0] + p, off[1] + p))

                    new_offsets.append((new_offsets[-1][1]+1, new_offsets[-1][1]+4))
                    context_word_offsets = q_offsets + new_offsets

                    if self.Is_training:
                        out_of_span = False
                        if self.opt['dataset'] == 'coqa' or self.opt['dataset'] == 'quac':
                            if tok_start_position<doc_span.start or tok_end_position>doc_span.start+doc_span.length:
                                out_of_span = True
                        else:
                        ...
                        if out_of_span:
                            ...
                            doc_offset = len(cur_query_token) + 2
                    ...
                    # if datum['qas'][i]['answer_span_start']==70 and datum['qas'][i]['answer_span_end']==91:
                    #     print(s, e)
                    #     print(start_position, end_position)
                    #     print(context_str[start_position:end_position])
                    #     print(answer)
                    #     print(datum['context'])
                    #     print(context_word_offsets)
                    #     exit(0)
                    # print(datum['context'][datum['qas'][i]['answer_span_start']: datum['qas'][i]['answer_span_end']])
                    # print(context_str[start_position: end_position])
                    # print("\n")
                    ...
                    if self.opt['dataset'] == 'coqa' or self.opt['dataset'] == 'quac':
                        span_answer_strs = datum['qas'][i]['span_text']
                        input_answer_strs = datum['qas'][i]['raw_answer']
