...
class BatchGen:
    def __init__(self, opt, data, use_cuda, vocab, evaluation=False):
    ...
    def __iter__(self):
        ...
        for datum in data:
            if not self.evaluation:
                ...
                datum['qas'] = [qa for qa in datum['qas'] if qa['answer_type'] == "extractive" and (
                                len(qa['annotated_answer']['word']) == 1 or qa['answer_span'][1] - qa['answer_span'][
                                    0] < MAX_ANS_SPAN)]
            ...
            for i in range(qa_len):
                ...
                cur_query_id = datum['qas'][i]['annotated_question']['wordid']
                cur_query_token = datum['qas'][i]['annotated_question']['word']
                if len(cur_query_id) > self.ques_max_len:
                    cur_query_id = cur_query_id[ :self.ques_max_len]
                    cur_query_token = cur_query_token[ :self.ques_max_len]
                ### 处理文章切片
                max_seq_length = self.opt['max_featrue_length']
                max_doc_length = max_seq_length - len(cur_query_id) - 3
                all_doc_tokens = datum['annotated_context']['wordid']
                ...
                ground_truth = torch.LongTensor(1, 2).fill_(-1)
                for (doc_index, doc_span) in enumerate(doc_spans):
                    ### 形成 [CLS] + Q + [SEP] + P + [SEP]

                    bert1, _ = self.bertify(datum['annotated_context']['word'])
                    bert2, _ = self.bertify(datum['context'])
                    print("bert1: ", len(bert1), bert1)
                    print("bert2: ", len(bert2), bert2)
                    print("context: ", len(datum['context']))
                    done
                    seq = cur_query_token + ['[SEP]'] + datum['annotated_context']['word'][doc_span.start: doc_span.start+doc_span.length]
                    half_seq_length = len(cur_query_token + ['[SEP]'])
                    if 'BERT' in self.opt:
                        x_bert, x_bert_offsets = self.bertify(seq)
                    ...
                    token_ids = [2] + cur_query_id + [3]
                    token_ids += datum['annotated_context']['wordid'][doc_span.start: doc_span.start+doc_span.length] + [3]
                    context_str = "".join(datum['annotated_context']['word'][doc_span.start: doc_span.start+doc_span.length])

                    x_len = len(token_ids)
                    x = torch.LongTensor(1, x_len).fill_(0)
                    x[0, :x_len] = torch.LongTensor(token_ids)
                    x_mask = 1 - torch.eq(x, 0)
                    # print("x_bert_mask: ", x_bert_mask.size(), x_bert_mask)
                    ...
                    # answer
                    ground_truth[0, 0] = datum['qas'][i]['answer_span'][0]
                    ground_truth[0, 1] = datum['qas'][i]['answer_span'][1]
                    answer = datum['qas'][i]['raw_answer']

                    if ground_truth[0, 0] >= 0 and ground_truth[0, 1] >= 0:
                        answer_str = answer
                    answer_strs = []
                    ...
                    for j in range(0, i):
                        query_id = datum['qas'][j]['annotated_question']['wordid']
                        query_token = datum['qas'][j]['annotated_question']['word']
                        answer_id = datum['qas'][j]['annotated_answer']['wordid']
                        answer_token = datum['qas'][j]['annotated_answer']['word']
                        if len(query_id) > self.ques_max_len:
                            query_id = query_id[ :self.ques_max_len]
                            query_token = query_token[ :self.ques_max_len]
                        if len(answer_id) > self.ans_max_len:
                            answer_id = answer_id[ :self.ans_max_len]
                            answer_token = answer_token[ :self.ans_max_len]

                        ques = query_token + ['[SEP]'] + answer_token
                        query, query_offsets = self.bertify(ques)
                        query_bert_mask = torch.LongTensor(1, len(query)).fill_(1)
                        query = torch.tensor([query], dtype = torch.long)
                        query_offsets = torch.tensor([query_offsets], dtype = torch.long)


                        # query_id = [2] + query_id + [3] + answer_id + [3]
                        # query = torch.LongTensor(1, len(query_id)).fill_(0)
                        # query[0, :len(query_id)] = torch.LongTensor(query_id)
                        ...
                        yield (
                        x_bert, x_bert_offsets, x_mask, x_bert_mask, query, query_mask, query_bert_mask,
                        ground_truth, context_str, context_words,
                        context_word_offsets, answer_strs, turn_ids)
